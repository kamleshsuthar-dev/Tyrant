...
class Category {
  async getAllCategory(req, res) {
    try {
      let Categories = await categoryModel.find({}).sort({ _id: -1 }).populate("cProducts");
  ...
  async postAddCategory(req, res) {
    ...
    if (!cName || !cDescription || !cStatus || !cImage) {
      ...
      if(!cName){error = "Category name is required"}
      else if(!cDescription){error = "Category description is required"}
      else if(!cStatus){error = "Category status is required"}
      else if(!cImage){error = "Category Image is required"}
  ...
  async postEditCategory(req, res) {
    try {
      ...
      if (!cId || !cName || !cDescription || !cStatus) {
        if (imageFile) {await deleteImage(imageFile.filename)}
        let error = "All fields must be filled";
        if (!cId) { error = "Category Id is required"; }
        else if (!cName) { error = "Category name is required"; }
        else if (!cDescription) { error = "Category description is required"; }
        else if (!cStatus) { error = "Category status is required"; }
        return res.status(400).json({
          success: false,
  ...
  async getDeleteCategory(req, res) {
    try {
      ...
      if (!cId) {return res.json({ error: "All fields must be filled" });} 
      else {
        ...
        if (deleteCategory) {
          ...
          deleteCategory.cProducts.forEach(async (product) => {
             if(!await productModel.findById(product)){
              ...
              if (deleteProduct) {
                deleteProduct.pImages.forEach(img => { deleteImage(img.ImageName) });
              }
            }
          });
